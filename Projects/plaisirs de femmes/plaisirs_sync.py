...
def parse_csv(path, cat_filter=None, log=None):
    ...
    count = 0
    # Pré-nettoyer les entités HTML qui contiennent ';' AVANT le parsing CSV.
    # Le CSV Matterhorn n'est pas quoté → les ';' dans &#39; &quot; etc. décalent
    # toutes les colonnes suivantes (wholesale_price lit la catégorie, etc.)
    with open(path, 'rb') as _fh:
        _raw = _fh.read().replace(b'\x00', b'')
    _raw = (_raw
            .replace(b'&#39;',   b"'")                      # apostrophe
            .replace(b'&quot;',  b"'")                      # guillemet → ' (csv-safe)
            .replace(b'&#34;',   b"'")                      # guillemet numérique
            .replace(b'&agrave;','à'.encode('utf-8'))
            .replace(b'&eacute;','é'.encode('utf-8'))
            .replace(b'&amp;',   b'&'))                     # ampersand (en dernier)
    with io.StringIO(_raw.decode(enc, errors='replace')) as f:
        ...
        for row in reader:
            ...
            # sizes_stock = quantités réelles (label:qty)
            # sizes_sku_codes = codes SKU (label:REF) – pas des quantités !
            sizes_str = (row.get('sizes_stock') or row.get('sizes_sku_codes') or
            ...
            # Si le champ contient des SKU non-numériques après ':', ignorer les qtés
            # (sizes_sku_codes peut passer ici si sizes_stock est vide)
            ...
            for e in sizes_str.split(','):
                if ':' in e:
                    ...
                    try:
                        ...
                        # SKU non-numérique (ex: sizes_sku_codes) → ignorer
                        if qs[:1].isalpha():
                            continue  # Ce champ contient des codes, pas des qtés
                        qty = 0
                    opts.append({'label': lbl, 'qty': qty, 'ean': '', 'sku': ''})

            dr = (row.get('description') or row.get('Description') or '').strip()
            # EAN : prendre le premier code de la liste (séparés par virgule ou espace)
            ean_raw = (row.get('ean_codes') or row.get('ean13') or row.get('EAN') or '').strip()
            ean13   = re.split(r'[,\s]+', ean_raw)[0][:13] if ean_raw else ''
            p = {
                'id':       pid,
...
def _download_ps_image(cur, conn, ps_id, img_urls, ps_img_dir, log=None):
    ...
    if not img_root.exists():
        if log: log.warn(f"  img_dir introuvable : {ps_img_dir}")
        return
    if not os.access(str(img_root), os.W_OK):
        if log: log.warn(f"  img_dir non accessible en écriture : {ps_img_dir}")
        return
    ...
    for rank, url in enumerate(img_urls[:4], start=1):
        if not url:
            continue
        try:
            r = _requests.get(url, timeout=10, allow_redirects=True)
            if r.status_code != 200:
                continue
            if len(r.content) < 2000:   # Pas une vraie image (page d'erreur)
                if log: log.warn(f"  Image rank={rank} trop petite ({len(r.content)} octets) : {url}")
                continue
            if not cover_done:
            ...
            # Rollback les INSERT ps_image non finalisés (fichier absent)
            try: conn.rollback()
...
def export_db(families, db_cfg, coef=2.2, tax_id=1, prefix='PDF', log=None, state=None,
    ...
    mfr_cache   = {}   # brand_name → id_manufacturer

    def _get_or_create_manufacturer(cur, conn, name):
        """Retourne id_manufacturer existant ou le crée."""
        if not name:
            return None
        name = name[:64]   # ps_manufacturer.name VARCHAR(64)
        if name in mfr_cache:
            return mfr_cache[name]
        cur.execute("SELECT id_manufacturer FROM ps_manufacturer WHERE name=%s", (name,))
        row = cur.fetchone()
        if row:
            mfr_cache[name] = row[0]
            return row[0]
        cur.execute(
            "INSERT INTO ps_manufacturer(name,active,date_add,date_upd) "
            "VALUES(%s,1,NOW(),NOW())", (name,))
        mid = cur.lastrowid
        cur.execute(
            "INSERT IGNORE INTO ps_manufacturer_lang"
            "(id_manufacturer,id_lang,description,short_description) "
            "VALUES(%s,1,'','')", (mid,))
        try:
            cur.execute(
                "INSERT IGNORE INTO ps_manufacturer_shop(id_manufacturer,id_shop)"
                " VALUES(%s,1)", (mid,))
        except Exception:
            pass
        conn.commit()
        ...
        try:
            ...
            if row:
                ...
                cur.execute(
                    ...
                    "(reference,price,wholesale_price,active,id_tax_rules_group,"
                    " isbn,upc,ean13,mpn,unity,date_add,date_upd) "
            ...
            brand  = main_p.get('brand', '').strip()
            id_mfr = _get_or_create_manufacturer(cur, conn, brand)
            if id_mfr:
                cur.execute(
                    "UPDATE ps_product SET id_manufacturer=%s WHERE id_product=%s",
                    (id_mfr, ps_id))

            # ── EAN13 ─────────────────────────────────────────────────────────
            ean13_val = main_p.get('ean13', '')
            if ean13_val:
                cur.execute(
                    "UPDATE ps_product SET ean13=%s WHERE id_product=%s",
                    (ean13_val, ps_id))
            ...
            if leaf:
                if leaf not in cat_cache:
                    ...
                    row = cur.fetchone()
                    if row:
                        cat_cache[leaf] = row[0]
                    else:
                        # Créer la catégorie sous "Accueil" (id_parent=2 en PS standard)
                        cur.execute(
                            "SELECT id_category FROM ps_category WHERE id_parent=2 AND active=1 LIMIT 1")
                        parent_row = cur.fetchone()
                        id_parent = parent_row[0] if parent_row else 2
                        cur.execute(
                            "INSERT INTO ps_category"
                            "(id_parent,level_depth,active,date_add,date_upd,is_root_category)"
                            " VALUES(%s,2,1,NOW(),NOW(),0)", (id_parent,))
                        new_cat_id = cur.lastrowid
                        lrewrite = slugify(leaf)[:128]
                        cur.execute(
                            "INSERT INTO ps_category_lang"
                            "(id_category,id_lang,id_shop,name,description,link_rewrite,meta_title,meta_description)"
                            " VALUES(%s,1,1,%s,'',%s,'','')", (new_cat_id, leaf, lrewrite))
                        try:
                            cur.execute(
                                "INSERT IGNORE INTO ps_category_shop(id_category,id_shop)"
                                " VALUES(%s,1)", (new_cat_id,))
                        except Exception:
                            pass
                        conn.commit()
                        cat_cache[leaf] = new_cat_id
                        if log: log.info(f"  Catégorie créée : '{leaf}' (id={new_cat_id})")
...
def _parser():
    ...
    p.add_argument('--no-images',  action='store_true',
                                   help='Désactive le téléchargement des images (import rapide)')
...
def main():
    ...
    ps_img_dir = None if args.no_images else (
        args.ps_img_dir or cfg.get('prestashop', {}).get('img_dir', '') or None
    )
