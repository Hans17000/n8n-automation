...
## ⚠️ DESIGN SYSTEM DISPONIBLE (mis à jour 12/06/2026)

Un design system complet a été livré par Claude Design dans `02-Dev/03-Design/`. **C'est la source de vérité visuelle.** Avant de coder quoi que ce soit, consulte ces fichiers :

### Tokens CSS — à importer directement dans le thème enfant
```
02-Dev/03-Design/tokens/colors.css       → palette complète + variantes + aliases sémantiques
02-Dev/03-Design/tokens/typography.css    → familles, tailles, poids, letter-spacings + @import Google Fonts
02-Dev/03-Design/tokens/spacing.css      → radii, bordures, espacements, transitions, layout
02-Dev/03-Design/styles.css              → point d'entrée (importe les 3 ci-dessus)
```

**Tu n'as PAS à recoder les variables CSS de la section 1 de ce brief.** Importe `styles.css` dans le thème enfant et utilise les variables qui y sont définies. Elles sont plus complètes que celles du brief initial (aliases sémantiques, variantes supplémentaires).

### Composants — référence visuelle (JSX → à traduire en Smarty/TPL + CSS)
Les composants dans `02-Dev/03-Design/components/` sont en React/JSX. Ils ne sont PAS à copier tel quel — PrestaShop utilise Smarty. Ils servent de **spécification visuelle** :
- `core/Button.jsx` → 4 variantes : primary, secondary, cabinet-entry, ghost
- `core/Badge.jsx` → « Pièce rare » (bordeaux) et « Cabinet » (prune)
- `core/PriceTag.jsx` → prix doré, Lato 300
- `core/SectionTitle.jsx` → titre Cormorant + sous-titre
- `core/EditorialText.jsx` → texte catégorie centré, italique, entre filets dorés
- `core/Separator.jsx` → filet doré
- `core/IrisQuote.jsx` → citation Pinyon Script
- `core/ReassuranceItem.jsx` → icône + texte réassurance
- `commerce/ProductCard.jsx` → miniature produit avec badge, hover, cadre
- `commerce/CategoryTile.jsx` → tuile catégorie vitrine/cabinet

### UI Kit — maquette de référence
`02-Dev/03-Design/ui_kits/boutique/index.html` est une maquette interactive complète de la boutique. Ouvre-la dans un navigateur pour voir exactement le rendu attendu : homepage, sas, cabinet, catégorie, fiche produit.

### Guidelines visuelles
`02-Dev/03-Design/guidelines/*.html` — spécimens de palette, typo, espacements, brand.

### Placeholders à noter
- **Photo hero** : pas encore fournie, aplat noir d'encre en attendant
- **Monogramme SVG** : placeholder typographique (« I » entre filets), SVG définitif à venir
- **Icônes custom** : Lucide utilisé en substitution, icônes gravure custom à venir
